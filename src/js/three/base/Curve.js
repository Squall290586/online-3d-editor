...
function checkLines(curve) {
  ...
  if (
    curve.childrens.length > 0 &&
    !curve.childrens
      .map(line => {
        return (
          curve.childrens.indexOf(line) === 0 ||
          curve.childrens[curve.childrens.indexOf(line) - 1].end
            .add(p)
            .equals(line.start.add(p))
        );
      })
...
class Curve extends Object {
  ...
  constructor(lines) {
    ...
    // Create the object
    super(new Position(), new THREE.Group());
  ...
  // Getters and setters
  get start() {
    return Optional.ofNullable(this.childrens[0])
      .map(v => v.start.add(this.position))
      .orElse(null);
  }

  get end() {
    return Optional.ofNullable(this.childrens.reduce((one, two) => two))
      .map(v => v.end.add(this.position))
      .orElse(null);
  }
